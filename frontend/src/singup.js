...
function validateEmail() {
  ...
  emailTimer = setTimeout(async () => {
    try {
      const res  = await fetch(`https://yourdomain.com/api/auth/check-email?email=${encodeURIComponent(val)}`);
...
function toggleAllAgree() {
  ...
  ['agreeTerms','agreePrivacy','agreeMedical','agreeMarketing'].forEach(id => {
...
function syncAllAgree() {
  const all = ['agreeTerms','agreePrivacy','agreeMedical','agreeMarketing']
...
async function handleRegister() {
  ...
  const agreeMedical  = document.getElementById('agreeMedical').checked;
  ...
  if (!agreeTerms || !agreePrivacy || !agreeMedical) { showFieldError('agreeError', '필수 약관에 모두 동의해주세요'); hasError = true; }
